...
impl FloatContext {
    ...
    pub fn place(&mut self, side: Float, width: f32, height: f32, y: f32) -> (f32, f32) {
        // §9.5.1 rule 6: a float's outer top may not be higher than the outer
        // top of any box generated by an earlier element — which for the boxes
        // this context knows about means any float already placed. Without it a
        // narrow float could slip back up into a gap beside an earlier, wider
        // one that had itself been pushed down, so a third float sat above the
        // second (`floats-141`).
        //
        // The maximum rather than the last, which is the same number while the
        // rule holds and does not depend on it holding.
        let earliest = self
            .floats
            .iter()
            .map(|float| float.top)
            .fold(f32::NEG_INFINITY, f32::max);
        let mut top = y.max(earliest);
...
mod tests {
    ...
    #[test]
    fn a_float_may_not_rise_above_an_earlier_one() {
        // §9.5.1 rule 6. A wide float pushed down leaves a gap beside the one
        // above it, and a later narrow float must not slip back up into it:
        // the third float here fits beside the first and still belongs below
        // the second (`floats-141`).
        let mut context = FloatContext::new(200.0);
        assert_eq!(context.place(Float::Left, 120.0, 10.0, 0.0), (0.0, 0.0));
        // No room beside the first, so this drops below it.
        assert_eq!(context.place(Float::Left, 120.0, 10.0, 0.0), (0.0, 10.0));
        // Room beside the *first* at y=0, and it may not go there.
        let (left, top) = context.place(Float::Left, 40.0, 10.0, 0.0);
        assert_eq!(top, 10.0, "it rose above the float before it");
        assert_eq!(left, 120.0, "beside the second, which it does fit next to");
    }
